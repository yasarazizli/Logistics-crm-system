...
const SelectAccountant = ({ modalClose, id, onSelect }: SelectManagerProps) => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      setLoader(true);
      const response = await getAllCm("accountant");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const formData = formCreator([
      { name: "accountant_id", data: selectedEmployee },
      { name: "role", data: "accountant" },
    ]);

    const { status, data } = await putManager(formData, id);
  ...
  return (
    <Modal title={t("cd.modal.title_1")} modalClose={() => modalClose(false)}>
      <form className={styles.form} onSubmit={handleSubmit}>
        <div className={styles.form__inputs}>
          <div className={styles.selectWrapper} style={{ marginTop: "10px" }}>
            <label className={styles.label}>{t("cd.modal.label_1")}</label>
            <select
              ...
              required
            >
              <option value="">{t("cd.modal.label_1")}</option>
