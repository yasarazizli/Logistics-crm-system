...
import { CountryDeleteRequest } from "@/features/dashboard/services/Controls/country.service.ts";
...
const DeleteDate = ({
  ...
  const deleteDate = async (event: FormEvent) => {
    ...
    setLoader(true);
    const { status, data } = await CountryDeleteRequest(id);
    if (status === 200) {
  ...
  return (
    <Modal
      title={t("workers.modals.delete.title__1")}
      subtitle={`${t("workers.modals.delete.subtitle__1")}`}
