      ...
      if (response?.status === 200) {
        ...
        if (response.data) {
          const matchedCargo = data.find(
            (item) =>
              item.cargo === response.data.cargo_name ||
              item.code === response.data.hs_code,
          );

          if (matchedCargo) {
            setSelectedCargo({
              label: matchedCargo.cargo,
              value: matchedCargo.id,
            });
            setSelectedCode({
              label: matchedCargo.code,
              value: matchedCargo.id,
            });
          } else {
            setSelectedCargo({
              label: response.data.cargo_name,
              value: 0,
            });
            setSelectedCode({
              label: response.data.hs_code,
              value: 0,
            });
          }
        }
